...
struct SoundscapeDetailView: View {
    ...
    var body: some View {
        ...
        .onChange(of: remainingTime) { _ in
            ...
            if remainingTime == 7 {
                fadeOutAudio() // Start fading out audio 7 seconds before the timer ends
            }
    ...
    // Fade out the audio over 7 seconds
    func fadeOutAudio() {
        let fadeDuration = 7.0
        let fadeStep = 0.1 // Reduce volume by 0.1 every step
        let fadeInterval = fadeDuration / (1.0 / fadeStep)

        Timer.scheduledTimer(withTimeInterval: fadeInterval, repeats: true) { timer in
            if self.soundscapePlayer.volume > 0.0 {
                self.soundscapePlayer.volume -= Float(fadeStep)
            } else {
                self.soundscapePlayer.stop()
                timer.invalidate()
            }
        }
    }
